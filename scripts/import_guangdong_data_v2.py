"""Re-import Guangdong data with 物理/历史 exam_category.

Imports:
  1. 一分一段 (score_rank_segments): 物理+历史 2025
  2. 投档数据 (admission_ranks): 物理2025 + 历史2026, with exam_category
"""
...
FILE_PHYSICS_ADMISSION = Path(
    "/home/sunnyxuan2/桌面/data for agent/附件2 广东省2025年本科普通类（物理）投档情况.xlsx"
)
...
def _parse_admission(filepath: Path, exam_category: str, year: int) -> list[dict]:
    ...
    # Dedup: keep best rank per college-year
    best: dict[str, dict] = {}
    for r in records:
        key = f"{r['college_id']}-{r['year']}-{r['exam_category']}"
        existing = best.get(key)
        if not existing or r["min_rank"] < existing["min_rank"] or (r["min_rank"] == 0 and existing["min_rank"] > 0):
            best[key] = r
...
def run():
    ...
    # --- 2. Import 投档数据 with exam_category ---
    print("\n=== Importing 投档数据 (admission_ranks) ===")
    total_ok = 0
    total_err = 0
    for label, fpath, exam_cat, year in [
        ("物理 2025", FILE_PHYSICS_ADMISSION, "物理", 2025),
    ...
    for cat in ("物理", "历史"):
        ...
        print(f"  {cat}: {cnt} admission records, {tc} total candidates")
    conn.close()
