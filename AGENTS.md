...
### Phase 23.5 🚀 专业大类双维度更新（2026-08-25 已实施）

> 状态：已完成 ✅（260 passed + tsc 通过）。用户指出专业大类需更新。调查发现 5 个问题：① majors 表被污染（516 id 中 479 个是组内序号 '001'=经济学…，仅 14 个 EN 码是标准专业）② category 29 类混杂（本科12门类+高职19大类+试验班/通用）③ **scorer 学科筛选语义错误**（majors.id 匹配组内序号 major_id，选"经济学"实际匹配所有 major_id='001' 的专业）④ 前端"学科门类"下拉 29 类直接展示 ⑤ college_major_name.category 仅 15281/40640 且混杂。
> 用户决策：本科 12 学科门类 + 教育部目录导入（`data for agent/教育部普通高等学校本科专业目录2026.docx`）+ **双维度**（门类 × 办学特征可组合）+ **多归属**（一专业可属多门类）。

#### 数据源（已解析验证）

教育部目录 DOCX → **852 专业 / 13 门类**（工学291/文学124/管理学73/艺术学67/医学62/法学53/农学51/理学50/教育学36/经济学30/历史学9/哲学4/交叉学科2）。解析要点：**段落+表格双通道**（医学等专业在段落 `100201K 临床医学` 格式）、名称去注释括号（"计算机科学与技术（注：可授工学或理学学士学位）"）、门类标记清洗（"文学05"→文学）。

#### 双维度模型

- **维度1 `category`（多归属）**：新建关联表 `college_major_category(college_id, major_id, category)`，一专业可属多门类
- **维度2 `program_type`**：`college_major_name` 加列（normal / 中外合作 ~197行 / 试验班 ~319行），可组合筛选（选"工学"+"中外合作"= 工学类合作项目）

#### 多归属归类规则

| 专业形态 | 归属 |
|---------|------|
| 目录注明"可授工学**或**理学学士"（计算机/心理学等） | 工学+理学 双归属（解析目录注释） |
| 人文科学试验班 | 文学+历史学+哲学 |
| 社会科学试验班 | 法学+经济学+管理学 |
| 经济管理试验班 | 经济学+管理学 |
| 工科/理科/文科试验班 | 单归（工学/理学/文学） |
| 中外合作 | 去括号按目录（通常单归） |
| 普通专业 | 目录：精确→去括号→包含→类名（"计算机类"→工学） |
| 未匹配 | 其他（单值） |
...
| # | 文件 | 改动 |
|---|------|------|
| 1 | `db.py` | 建 `college_major_category` 关联表 + `college_major_name` 幂等加 `program_type TEXT DEFAULT 'normal'` |
| 2 | `scripts/seed_major_categories.py`（新建） | 解析目录 DOCX（段落+表格双通道、去注释括号、门类标记清洗；**交叉学科为教育部 2026 新增门类，保留**）+ 双维度归类回填（39354 有名行；关联表多归属 + program_type），dry-run + 备份 |
| 3 | `volunteer_scorer.py` | 过滤修复：`major_category_ids` 改 **(college_id, major_id) 集合**（来源换 `college_major_category` 关联表）；新增 `program_type` 参数；**组级过滤：门类/特征筛选时组内专业全被筛掉则移除整组** |
| 4 | `volunteer.py` | `/major-categories` 返回固定 16 类（12门类+交叉学科+中外合作+试验班+其他）；Browse/Recommend 加 `program_type` |
| 5 | `volunteer_table.py` | CreatePlanRequest 加 `program_type` |
| 6 | `page.tsx` | 学科门类下拉（16 类）+ "办学特征"下拉（全部/普通/中外合作/试验班）+ 三处请求透传 + deps |
| 7 | majors 表 | 删除 479 个数字 id 污染行（剩 37：14 EN + 23 其他；subject_major_map 用 EN 码不受影响） |

#### 执行结果

- 回填：39354 行 program_type（normal 38938 / 试验班 318 / 中外合作 98）+ 关联表归属
- category 归属：工学 11879 / 其他 9063（**多为专科专业名**，本科目录无对应，归其他合理）/ 管理学 5182 / 理学 3613 / 文学 2726 / 医学 2717 / 艺术学 2284 / 经济学 1772 / 法学 1403 / 教育学 1207 / 农学 653 / 历史学 113 / 哲学 46 / 交叉学科 3；未匹配率 21.2%
- **中外合作 98 行全在艺体/提前批**（艺术学 87 + 教育学 10 + 其他 1），不在本科批投档——本科批选"中外合作"为 0 属数据实际，非 bug

#### 验证

- `pytest` **260 passed** + `tsc --noEmit`；备份 `bak_before_major_cat_20260825_*`
- 端到端：`/major-categories` 16 类；选"工学"→ 全工学（环境/数据/机械/土木）；"试验班"→ 51 组（医学/社科/经管/工科/理科试验班）；"工学+试验班"→ 34 组全工科试验班（组合筛选正确）
- Playwright：学科门类下拉（含工学/中外合作按钮）+ 办学特征下拉（全部/普通/中外合作/试验班）渲染 + 试验班筛选查询正常

#### 踩坑

- **classify 类名匹配返回裸字符串**（`return cands[0][1]`，class_map 值是 str）→ 迭代逐字符拆成单字脏类别 → 改为 `return [cands[0][1]]`
- **program_type 单独筛选时组不降级**（原 valid_groups 过滤仅门类筛选触发）→ 空专业组残留，改为门类/特征任一筛选都做组级过滤
- 目录解析三处：段落+表格双通道（医学在段落 `100201K 临床医学`）、去注释括号（"计算机科学与技术（注：可授工学或理学学士学位）"）、门类标记清洗（"文学05"→文学）

#### 风险与兜底

- 旧 category（15281 混杂值）被覆盖 → DB 备份可回滚
- 少数试验班（名称无学科线索）归"其他" → 仍可通过"办学特征=试验班"完整筛出

### Phase 23.6 🚀 官方志愿样表导出（Excel+PDF，2026-08-25 已实施）

> 状态：已完成 ✅（260 passed + tsc 通过）。用户提供官方《2026年广东省普通高校招生考生志愿表》样表（`data for agent/2026年广东省高考志愿样表.xlsx`，211行×15列），要求按官方格式一键导出志愿表。系统原 export_service（自定义格式：方案名/概率/标签）替换为官方样表格式。

#### 官方样表结构（已解析）

- 标题"2026年广东省普通高校招生考生志愿表" + 考生号填写区 + 表头（录取批次|批次类型|志愿号|院校代码|院校名称|院校专业组代码|专业代码1-6|是否服从调剂）
- 官方固定批次段：提前批（招飞1/军检10/艺术统考+校考1/戏曲联考1/非军检20/教师10/卫生10/特殊类型1）→ 专科提前（定向军士10/专科卫生10）→ 本科批（艺体统考20/普通类本科45，分段显示）→ 专科批（艺体20/普通类45）+ 尾注3条

#### 方案：模板填充（100% 保真）

| # | 改动 | 说明 |
|---|------|------|
| 1 | 样表入项目 | 官方 xlsx 拷贝至 `data/user/custom/templates/2026_gd_volunteer_form.xlsx` 作模板资源 |
| 2 | `export_service.py` 改造 | openpyxl 加载官方模板 → 按 plan.batch 定位批次段 → 填 slots → 输出（合并单元格/列宽/样式全保留） |
| 3 | 填写映射 | 院校代码=slot.province_code（广东招生代码）；专业组代码=group_code；**专业代码=major_id.split('-')[-1]**（艺体类 major_id 为 `{组号}-{代码}` 命名空间需去前缀，本科/提前批直接用）；服从调剂=adjustable；未填志愿行留空 |
| 4 | PDF | soffice 转换填好的 xlsx → PDF（与 Excel 格式完全一致） |
| 5 | API/前端 | `export-pdf`/`export-excel` 端点改走官方格式；按钮文案改"导出志愿表(官方样表)" |
...
| 提前批·空军海军招飞 | 招飞段 | 1 |
| 艺体类本科批 | 艺体类本科段 | 1-20 |
| 提前批·特殊类型 | 特殊类型段（按 special_type 定位高水平运动队/综合评价子行） | 1 |

#### 执行结果（2026-08-25）

- 模板入库：`data/user/custom/templates/2026_gd_volunteer_form.xlsx`；批次段行号实测（招飞 r6 / 军检 r7-16 / 艺术统考+校考 r17 / 戏曲 r18 / 非军检 r19-38 / 教师 r39-48 / 卫生 r49-58 / 特殊类型 高水平 r59·综合评价 r60 / 专科军士 r61-70 / 专科卫生 r71-80 / 艺体本科 r81-100 / 普通本科 r101-145 / 艺体专科 r146-165 / 普通专科 r166-210）
- `export_service.py` 追加 `export_plan_official_excel`（模板填充：F院校代码=province_code / G名称 / H组号=group_code / I-N专业=major_id.split('-')[-1] / O服从调剂；未填段留空；合并单元格 40 个全保留）+ `export_plan_official_pdf`（soffice --headless 转 PDF）
- 特殊类型子行定位：slots 无 special_type 字段 → 导出时查 `colleges.special_type`
- API `export-pdf`/`export-excel` 改走官方格式；前端按钮文案"志愿表PDF/志愿表Excel"
- **验证**：本科批 45 slots 方案导出 → Excel 普通类本科段 r101 起填入（10561 华工 209 组 061/058 服从 ✓），军检段留空 ✓，PDF 116KB 渲染对照官方布局（提前批留空、本科段填入、列宽与官方一致）；`pytest` **260 passed**；`tsc --noEmit` 通过
- 已知限制：院校名称列宽为官方手写表宽度，PDF 显示可能截断（Excel 内完整）——保持官方保真不改
